...
import { loadFollowCandidates, selectApprovedFollowCandidates } from '../../workflows/plan-follow.js';
...
export function registerFollowCommands(program: Command): void {
  program
    .command('follow')
    .description('Follow supervisionado. Padrão: dry-run. Ações reais exigem plano e limite positivo.')
    ...
    .option('--skip-inactive <n>', 'pula perfis com menos de N seguidores E menos de N seguindo')
    ...
    .action(async (options: {
      ...
      try {
        ...
        if (options.plan) {
          ...
          if (mode !== 'dry-run') {
            write({ error: 'Ações reais exigem um plano congelado (--plan). Use plan:create-follow.' });
            process.exitCode = 1;
            return;
          }
