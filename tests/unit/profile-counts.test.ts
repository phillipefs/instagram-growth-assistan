...
describe('extractProfileCounts', () => {
  ...
  it('lê seguidores e seguindo (português)', () => {
    ...
    expect(counts).toEqual({ followers: 150, following: 80 });
  });

  it('ignora o botão "Following" sem número antes', () => {
    const counts = extractProfileCounts('username Following Message 5 posts 900 followers 300 following');
    expect(counts).toEqual({ followers: 900, following: 300 });
  });

  it('lê singular "1 follower"', () => {
    const counts = extractProfileCounts('x 0 posts 1 follower 0 following');
    expect(counts).toEqual({ followers: 1, following: 0 });
  });

  it('retorna null quando não encontra', () => {
    expect(extractProfileCounts('nada aqui')).toEqual({ followers: null, following: null });
  });
});
